...
{
    public partial class PlanYourWeekendMenMedium : Form
    {
        private Timer timer;
        ...
        public PlanYourWeekendMenMedium()
        {
            InitializeComponent();
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SaveChallengeDataFailed(Login.CurrentUsername, textBox2.Text, "Mental Weekend (Medium)");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (button1.Text == "DONE")
            {
                ...
                SaveChallengeDataSuccess(Login.CurrentUsername, "Completed", textBox2.Text, "Mental Weekend (Medium)");
        ...
        private InstructionsPYWMM InstructionPYWMM = null; // Class-level variable to track the Guide form
        ...
        private void Instructions_Click(object sender, EventArgs e)
        {
            ...
            if (InstructionPYWMM != null && !InstructionPYWMM.IsDisposed)
            {
                InstructionPYWMM.BringToFront();
                return;
            }

            // Create the Guide form
            InstructionPYWMM = new InstructionsPYWMM();
            InstructionPYWMM.FormBorderStyle = FormBorderStyle.FixedToolWindow; // Set to FixedToolWindow
            InstructionPYWMM.StartPosition = FormStartPosition.CenterScreen;
            InstructionPYWMM.Opacity = 0; // Start at 0 for fade-in effect

            // Add functionality to fade out and close the Guide form when the X button is clicked
            InstructionPYWMM.FormClosing += (s, ev) =>
            {
                ...
                {
                    if (InstructionPYWMM.Opacity > 0)
                    {
                        InstructionPYWMM.Opacity -= 0.05; // Decrease opacity for fade-out
                    }
                    else
                    {
                        fadeOutTimer.Stop();
                        fadeOutTimer.Dispose();
                        InstructionPYWMM.FormClosing -= null; // Remove event handler to avoid recursion
                        InstructionPYWMM.Dispose(); // Dispose of the Guide form
                        InstructionPYWMM = null; // Reset the reference
                    }
                };
                fadeOutTimer.Start();
            };
            ...
            {
                if (InstructionPYWMM.Opacity < 1)
                {
                    InstructionPYWMM.Opacity += 0.05; // Increase opacity for fade-in
                }
                else
                {
                    fadeInTimer.Stop();
