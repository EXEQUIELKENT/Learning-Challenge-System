...
{
    public partial class PlanYourWeekendMenHard : Form
    {
        private Timer timer;
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SaveChallengeDataFailed(Login.CurrentUsername, textBox2.Text, "Mental Weekend (Hard)");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (button1.Text == "DONE")
            {
                ...
                SaveChallengeDataSuccess(Login.CurrentUsername, "Completed", textBox2.Text, "Mental Weekend (Hard)");
        ...
        private InstructionsPYWMH InstructionsPYWMH = null; // Class-level variable to track the Guide form
        ...
        private void Instructions_Click(object sender, EventArgs e)
        {
            ...
            if (InstructionsPYWMH != null && !InstructionsPYWMH.IsDisposed)
            {
                InstructionsPYWMH.BringToFront();
                return;
            }

            // Create the Guide form
            InstructionsPYWMH = new InstructionsPYWMH();
            InstructionsPYWMH.FormBorderStyle = FormBorderStyle.FixedToolWindow; // Set to FixedToolWindow
            InstructionsPYWMH.StartPosition = FormStartPosition.CenterScreen;
            InstructionsPYWMH.Opacity = 0; // Start at 0 for fade-in effect

            // Add functionality to fade out and close the Guide form when the X button is clicked
            InstructionsPYWMH.FormClosing += (s, ev) =>
            {
                ...
                {
                    if (InstructionsPYWMH.Opacity > 0)
                    {
                        InstructionsPYWMH.Opacity -= 0.05; // Decrease opacity for fade-out
                    }
                    else
                    {
                        fadeOutTimer.Stop();
                        fadeOutTimer.Dispose();
                        InstructionsPYWMH.FormClosing -= null; // Remove event handler to avoid recursion
                        InstructionsPYWMH.Dispose(); // Dispose of the Guide form
                        InstructionsPYWMH = null; // Reset the reference
                    }
                };
                fadeOutTimer.Start();
            };
            ...
            {
                if (InstructionsPYWMH.Opacity < 1)
                {
                    InstructionsPYWMH.Opacity += 0.05; // Increase opacity for fade-in
                }
                else
                {
                    fadeInTimer.Stop();
