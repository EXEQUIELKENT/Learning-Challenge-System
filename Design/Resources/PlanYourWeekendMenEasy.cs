...
{
    public partial class PlanYourWeekendMenEasy : Form
    {
        private Timer timer;
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SaveChallengeDataFailed(Login.CurrentUsername, textBox2.Text, "Mental Weekend (Easy)");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (button1.Text == "DONE")
            {
                ...
                if (MondayrichTextBox.Text.Length <= 50 ||
                    TuesdayrichTextBox.Text.Length <= 50 ||
                    WednesdayrichTextBox.Text.Length <= 50 ||
                    ThursdayrichTextBox.Text.Length <= 50 ||
                    FridayrichTextBox.Text.Length <= 50 ||
                    SaturdayrichTextBox.Text.Length <= 50 ||
                    SundayrichTextBox.Text.Length <= 50)
                {
                    MessageBox.Show("Each day's plan must be at least 50 characters long to complete the challenge.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                SaveChallengeDataSuccess(Login.CurrentUsername, "Completed", textBox2.Text, "Mental Weekend (Easy)");
        ...
        private InstructionsPYWME InstructionsPYWME = null; // Class-level variable to track the Guide form
        ...
        private void Instructions_Click(object sender, EventArgs e)
        {
            ...
            if (InstructionsPYWME != null && !InstructionsPYWME.IsDisposed)
            {
                InstructionsPYWME.BringToFront();
                return;
            }

            // Create the Guide form
            InstructionsPYWME = new InstructionsPYWME();
            InstructionsPYWME.FormBorderStyle = FormBorderStyle.FixedToolWindow; // Set to FixedToolWindow
            InstructionsPYWME.StartPosition = FormStartPosition.CenterScreen;
            InstructionsPYWME.Opacity = 0; // Start at 0 for fade-in effect

            // Add functionality to fade out and close the Guide form when the X button is clicked
            InstructionsPYWME.FormClosing += (s, ev) =>
            {
                ...
                {
                    if (InstructionsPYWME.Opacity > 0)
                    {
                        InstructionsPYWME.Opacity -= 0.05; // Decrease opacity for fade-out
                    }
                    else
                    {
                        fadeOutTimer.Stop();
                        fadeOutTimer.Dispose();
                        InstructionsPYWME.FormClosing -= null; // Remove event handler to avoid recursion
                        InstructionsPYWME.Dispose(); // Dispose of the Guide form
                        InstructionsPYWME = null; // Reset the reference
                    }
                };
                fadeOutTimer.Start();
            };
            ...
            {
                if (InstructionsPYWME.Opacity < 1)
                {
                    InstructionsPYWME.Opacity += 0.05; // Increase opacity for fade-in
                }
                else
                {
                    fadeInTimer.Stop();
        ...
        private void PlanYourWeekendMenEasy_Load(object sender, EventArgs e)
        {
