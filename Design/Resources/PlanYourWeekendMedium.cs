...
{
    public partial class PlanYourWeekendMedium : Form
    {
        private Timer timer;
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SaveChallengeDataFailed(Login.CurrentUsername, textBox2.Text, "Plan Your Weekend (Medium)");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (button1.Text == "DONE")
            {
                ...
                if (MondayrichTextBox.Text.Length <= 100 ||
                    TuesdayrichTextBox.Text.Length <= 100 ||
                    WednesdayrichTextBox.Text.Length <= 100 ||
                    ThursdayrichTextBox.Text.Length <= 100 ||
                    FridayrichTextBox.Text.Length <= 100 ||
                    SaturdayrichTextBox.Text.Length <= 100 ||
                    SundayrichTextBox.Text.Length <= 100)
                {
                    MessageBox.Show("Each day's plan must be at least 100 characters long to complete the challenge.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                SaveChallengeDataSuccess(Login.CurrentUsername, "Completed", textBox2.Text, "Plan Your Weekend (Medium)");
        ...
        private InstructionsPYWM InstructionsPYWM = null; // Class-level variable to track the Guide form
        ...
        private void Instructions_Click(object sender, EventArgs e)
        {
            ...
            if (InstructionsPYWM != null && !InstructionsPYWM.IsDisposed)
            {
                InstructionsPYWM.BringToFront();
                return;
            }

            // Create the Guide form
            InstructionsPYWM = new InstructionsPYWM();
            InstructionsPYWM.FormBorderStyle = FormBorderStyle.FixedToolWindow; // Set to FixedToolWindow
            InstructionsPYWM.StartPosition = FormStartPosition.CenterScreen;
            InstructionsPYWM.Opacity = 0; // Start at 0 for fade-in effect

            // Add functionality to fade out and close the Guide form when the X button is clicked
            InstructionsPYWM.FormClosing += (s, ev) =>
            {
                ...
                {
                    if (InstructionsPYWM.Opacity > 0)
                    {
                        InstructionsPYWM.Opacity -= 0.05; // Decrease opacity for fade-out
                    }
                    else
                    {
                        fadeOutTimer.Stop();
                        fadeOutTimer.Dispose();
                        InstructionsPYWM.FormClosing -= null; // Remove event handler to avoid recursion
                        InstructionsPYWM.Dispose(); // Dispose of the Guide form
                        InstructionsPYWM = null; // Reset the reference
                    }
                };
                fadeOutTimer.Start();
            };
            ...
            {
                if (InstructionsPYWM.Opacity < 1)
                {
                    InstructionsPYWM.Opacity += 0.05; // Increase opacity for fade-in
                }
                else
                {
                    fadeInTimer.Stop();
