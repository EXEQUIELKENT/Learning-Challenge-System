...
{
    public partial class PlanYourWeekendEasy : Form
    {
        private Timer timer;
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SaveChallengeDataFailed(Login.CurrentUsername, textBox2.Text, "Plan Your Weekend (Easy)");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (button1.Text == "DONE")
            {
                ...
                SaveChallengeDataSuccess(Login.CurrentUsername, "Completed", textBox2.Text, "Plan Your Weekend (Easy)");
        ...
        private InstructionsPYWE instructionsPYWE = null; // Class-level variable to track the Guide form
        ...
        private void Instructions_Click(object sender, EventArgs e)
        {
            ...
            if (instructionsPYWE != null && !instructionsPYWE.IsDisposed)
            {
                instructionsPYWE.BringToFront();
                return;
            }

            // Create the Guide form
            instructionsPYWE = new InstructionsPYWE();
            instructionsPYWE.FormBorderStyle = FormBorderStyle.FixedToolWindow; // Set to FixedToolWindow
            instructionsPYWE.StartPosition = FormStartPosition.CenterScreen;
            instructionsPYWE.Opacity = 0; // Start at 0 for fade-in effect

            // Add functionality to fade out and close the Guide form when the X button is clicked
            instructionsPYWE.FormClosing += (s, ev) =>
            {
                ...
                {
                    if (instructionsPYWE.Opacity > 0)
                    {
                        instructionsPYWE.Opacity -= 0.05; // Decrease opacity for fade-out
                    }
                    else
                    {
                        fadeOutTimer.Stop();
                        fadeOutTimer.Dispose();
                        instructionsPYWE.FormClosing -= null; // Remove event handler to avoid recursion
                        instructionsPYWE.Dispose(); // Dispose of the Guide form
                        instructionsPYWE = null; // Reset the reference
                    }
                };
                fadeOutTimer.Start();
            };
            ...
            {
                if (instructionsPYWE.Opacity < 1)
                {
                    instructionsPYWE.Opacity += 0.05; // Increase opacity for fade-in
                }
                else
                {
                    fadeInTimer.Stop();
        ...
        private void PlanYourWeekendEasy_Load(object sender, EventArgs e)
        {
