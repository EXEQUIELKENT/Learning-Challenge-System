...
namespace DCP
{
    public partial class HydrationMedium : Form
    {
        private Timer timer;
        private int progressDuration = 60; // total time for progress bar in seconds
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                textBox2.Text = "00:00:30"; // Set the final time

                string timeTaken = textBox2.Text; // Use the time from the TextBox

                // Save challenge data to JSON file
                if (int.TryParse(textBox1.Text, out int glass))
                {
                    SaveChallengeDataFailed(Login.CurrentUsername, glass, timeTaken);
                }
        ...
        public void SaveChallengeDataFailed(string username, int glass, string time)
        {
            ...
            {
                Date = date,
                FormTitle = "Hydration (Medium)",
                Glass = $"Failed {glass}", // For PushUpEasy
                Time = time
            };
        ...
        public void SaveChallengeDataSuccess(string username, int glass, string time)
        {
            // File path for the challenge data
            string date = DateTime.Now.ToString("MM-dd-yy");
            string challengeFilePath = $"{username}_challenge.json";
            ...
            var newChallengeData = new
            {
                Date = date,
                FormTitle = "Hydration (Medium)",
                Glass = $"Completed {glass}", // For PushUpEasy
                Time = time
            };
        ...
        private void HydrationEasy_Load(object sender, EventArgs e)
        {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Type how many glass you've drank.", "No Glass Entered", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (int.TryParse(textBox1.Text, out int glass))
            {
                ...
                if (glass < 4)
                {
                    ...
                    count.Stop();
                    fail.Play();
                    message = "Challenge not completed. Returning to the homepage.";
                ...
                {
                    SaveChallengeDataSuccess(Login.CurrentUsername, glass, timeTaken);
                ...
                if (timer != null && timer.Enabled) // Assuming 'challengeTimer' is your timer
                {
                    ...
                    if (result2 == DialogResult.Yes)
                    {
                        if (int.TryParse(textBox1.Text, out int glass))
                        {
                            ...
                            string timeTaken = textBox2.Text; // Use the time from the TextBox
                                                              // Save challenge data to JSON file
                            SaveChallengeDataFailed(Login.CurrentUsername, glass, textBox2.Text);
        ...
        private void HydrationMedium_Load(object sender, EventArgs e)
        {
            textBox1.KeyPress += textBox1_KeyPress;
        }
    }
